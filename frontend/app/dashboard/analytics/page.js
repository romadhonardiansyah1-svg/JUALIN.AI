...
export default function AnalyticsPage() {
  ...
  const metrics = [
    ...
    { label: "Conversion Rate", value: "46%", change: "+5%", border: "var(--stat-orange)", bg: "var(--stat-orange-bg)", icon: "🎯" },
  ];
  ...
  const dailyOrders = [5, 8, 12, 10, 15, 11, 18, 14, 20, 16, 22, 19, 15, 18, 21, 17, 12, 14, 16, 20, 23, 18, 15, 19, 22, 25, 20, 18, 23, 21];
    ...
    <div className={styles.analyticsPage}>
      ...
      <div className="card">
        ...
        <div className={styles.trendChart}>
          {dailyOrders.map((v, i) => (
            <div key={i} className={styles.trendBar} style={{ height: `${(v / 25) * 100}%` }} title={`Hari ${i + 1}: ${v} order`}></div>
          ))}
        </div>
      </div>
            ...
            {topProducts.map((p, i) => (
              <div key={i} className={styles.topProduct}>
                ...
                <div className={styles.topBarWrap}>
                  <div className={styles.topBar} style={{ width: `${(p.count / 87) * 100}%` }}></div>
                </div>
        ...
        <div className="card">
          ...
          <div className={styles.aiStats}>
            <div className={styles.aiStatItem}>
              ...
              <div>
                <div className={styles.aiStatValue}>3 detik</div>
            ...
            <div className={styles.aiStatItem}>
              <span className={styles.aiStatIcon}>🛒</span>
              <div>
                <div className={styles.aiStatValue}>46%</div>
          ...
          <div className={styles.quotaSection}>
            ...
            <div className={styles.quotaRow}>
              ...
              <span>{quota?.limit || 0} limit (Pro)</span>
            </div>
            <div className="progress-bar">
              <div className="progress-fill" style={{ width: `${quota?.percentage || 0}%` }}></div>
            </div>
            <p className="text-sm text-muted mt-2">✅ Estimasi cukup sampai akhir bulan</p>
          </div>
        </div>
      </div>
    </div>
