...
def upgrade():
    ...
    def add_col_if_not_exists(table, col):
        if not _has_column(table, col.name):
            op.add_column(table, col)

    add_col_if_not_exists("agent_policies", sa.Column("version", sa.Integer(), nullable=False, server_default="1"))
    add_col_if_not_exists("agent_policies", sa.Column("payment_recovery_mode", sa.String(length=30), nullable=False, server_default="observe"))
    add_col_if_not_exists("agent_policies", sa.Column("payment_recovery_paused", sa.Boolean(), nullable=False, server_default=sa.text("true")))
    add_col_if_not_exists("agent_policies", sa.Column("timezone", sa.String(length=50), nullable=False, server_default="Asia/Jakarta"))
    add_col_if_not_exists("agent_policies", sa.Column("quiet_hours_start", sa.Time(), nullable=True))
    add_col_if_not_exists("agent_policies", sa.Column("quiet_hours_end", sa.Time(), nullable=True))
    add_col_if_not_exists("agent_policies", sa.Column("daily_recipient_cap", sa.Integer(), nullable=False, server_default="1"))
    add_col_if_not_exists("agent_policies", sa.Column("order_cycle_cap", sa.Integer(), nullable=False, server_default="1"))
    add_col_if_not_exists("agent_policies", sa.Column("cooldown_minutes", sa.Integer(), nullable=False, server_default="1440"))

    # ── BackgroundJob: lease/fencing and safety fields ──
    add_col_if_not_exists("background_jobs", sa.Column("lease_expires_at", sa.DateTime(timezone=True), nullable=True))
    add_col_if_not_exists("background_jobs", sa.Column("claim_token", postgresql.UUID(as_uuid=True), nullable=True))
    add_col_if_not_exists("background_jobs", sa.Column("lock_version", sa.Integer(), nullable=False, server_default="0"))
    add_col_if_not_exists("background_jobs", sa.Column("execution_stage", sa.String(length=30), nullable=False, server_default="unknown"))
    add_col_if_not_exists("background_jobs", sa.Column("side_effect_started_at", sa.DateTime(timezone=True), nullable=True))
    add_col_if_not_exists("background_jobs", sa.Column("payload_digest", sa.String(length=64), nullable=True))
    add_col_if_not_exists("background_jobs", sa.Column("handler_contract_version", sa.Integer(), nullable=True))
    ...
    if not _has_index("background_jobs", "ix_jobs_processable"):
        op.create_index(
            "ix_jobs_processable",
            "background_jobs",
    ...
    # Partial index for queued processable
    if not _has_index("background_jobs", "ix_jobs_queued_processable"):
        try:
            op.create_index(
    ...
    if not _has_column("webhook_events", "seller_id"):
        op.add_column("webhook_events", sa.Column("seller_id", sa.Integer(), nullable=True))
    if not _has_column("webhook_events", "provider_account_id"):
        op.add_column("webhook_events", sa.Column("provider_account_id", sa.String(length=255), nullable=True))
    if not _has_column("webhook_events", "channel_id"):
        op.add_column("webhook_events", sa.Column("channel_id", sa.Integer(), nullable=True))

    if not _has_index("webhook_events", "ix_webhook_events_seller_id"):
        op.create_index("ix_webhook_events_seller_id", "webhook_events", ["seller_id"], unique=False)
    if not _has_index("webhook_events", "ix_webhook_events_provider_account_id"):
        op.create_index("ix_webhook_events_provider_account_id", "webhook_events", ["provider_account_id"], unique=False)
    if not _has_index("webhook_events", "ix_webhook_events_channel_id"):
    ...
    if not _has_table("payment_attempts"):
        op.create_table(
            "payment_attempts",
        ...
        op.execute(
            """
            CREATE UNIQUE INDEX IF NOT EXISTS uq_payment_attempt_one_current_per_order
